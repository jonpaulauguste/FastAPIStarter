...
PLACE_DETAILS_OVERRIDES = {
    ...
    "Barry's Gyro": {
        "location": "TLC Courtyard",
        "description": "Authentic Greek-style gyros, wraps, and grilled meats served fresh with homemade sauces.",
    },
    "Island Bites": {
        "location": "TLC Food Court",
        "description": "Caribbean lunch plates with jerk chicken, rice, slaw, and island sides.",
    },
}

REQUIRED_MANAGER_PLACES = [
    {
        "name": "Barry's Gyro",
        ...
        "description": "Authentic Greek-style gyros, wraps, and grilled meats served fresh with homemade sauces.",
        "rating": 4.4,
        "image_url": "/static/img/barrys-gyro.jpg",
        "menu_items": [
            {"name": "Classic Lamb Gyro", "price": 30.0},
            {"name": "Chicken Souvlaki Plate", "price": 36.0},
            {"name": "Greek Fries", "price": 18.0},
        ],
    },
    {
        "name": "Island Bites",
        ...
        "description": "Caribbean lunch plates with jerk chicken, rice, slaw, and island sides.",
        "rating": 4.6,
        "image_url": "/static/img/island-bites.jpg",
        "menu_items": [
            {"name": "Jerk Chicken Plate", "price": 42.0},
            {"name": "Stew Chicken with Rice", "price": 38.0},
            {"name": "Plantain Chips", "price": 12.0},
        ],
    },
]
...
def _seed_places_and_menu(session: Session):
    ...
    menu_items = [
        ...
        MenuItem(name="Classic Lamb Gyro", price=30.0, place_id=places[6].id),
        MenuItem(name="Chicken Souvlaki Plate", price=36.0, place_id=places[6].id),
        MenuItem(name="Greek Fries", price=18.0, place_id=places[6].id),
        MenuItem(name="Jerk Chicken Plate", price=42.0, place_id=places[7].id),
        MenuItem(name="Stew Chicken with Rice", price=38.0, place_id=places[7].id),
        MenuItem(name="Plantain Chips", price=12.0, place_id=places[7].id),
    ]
    session.add_all(menu_items)


def _ensure_required_places_and_menu(session: Session):
    for place_data in REQUIRED_MANAGER_PLACES:
        place = session.exec(select(Place).where(Place.name == place_data["name"])).first()
        ...
        if place is None:
            ...
            place.cuisine = place_data["cuisine"]
            if not place.location.strip():
                place.location = place_data["location"]
            if not place.description.strip():
                place.description = place_data["description"]
            if not place.image_url.strip():
        ...
        existing_menu_names = {
            ...
            for item in session.exec(
                select(MenuItem).where(MenuItem.place_id == place.id)
            ).all()
        }

        for menu_data in place_data["menu_items"]:
            if menu_data["name"] in existing_menu_names:
                continue
...
def _seed_reviews(session: Session, bob: User, student: User):
    ...
    review_seed_by_place = {
        ...
        "Barry's Gyro": {
            "rating": 4,
            "comment": "Flavorful gyros and great sauces.",
            "user_id": student.id,
        },
        "Island Bites": {
            "rating": 5,
            "comment": "Excellent island flavors and generous portions.",
            "user_id": bob.id,
        },
    }
...
def create_db_and_tables():
    ...
    with Session(engine) as session:
        ...
        _ensure_required_places_and_menu(session)
